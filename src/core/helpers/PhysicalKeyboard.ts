...
import { IPhysicalKeyboardParams } from '@/core/interfaces/physicalKeyboard.interfaces';
...
class PhysicalKeyboard {
   ...
   */
  constructor({ dispatch, getOptions }: IPhysicalKeyboardParams) {
    /**
     ...
     * Bindings
     */
    InputUtilities.bindMethods(PhysicalKeyboard, this);
  }
  ...
  handleHighlightKeyDown(event: KeyboardEvent): void {
    ...
    this.dispatch((instance: any) => {
      ...
      if(buttonDOM) {
        buttonDOM.style.background = options.physicalKeyboardHighlightBgColor || `#dadce4`;
  ...
  handleHighlightKeyUp(event: KeyboardEvent): void {
    ...
    this.dispatch((instance: any) => {
      const buttonDOM = instance.getButtonElement(buttonPressed)
