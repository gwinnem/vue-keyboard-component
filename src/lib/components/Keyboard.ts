...
// Services
import { getDefaultLayout } from '../../core/helpers/KeyboardLayout';
import PhysicalKeyboard from '../../core/helpers/PhysicalKeyboard';
import InputUtilities from '../../core/helpers/InputUtilities';
import {
  IKeyboardOptions,
  IKeyboardInput,
  IKeyboardButtonElements,
} from '@/core/interfaces/keyboard.interfaces';
import CandidateBox from './CandidateBox';
import { TKeyboardElement, TKeyboardHandlerEvent } from '@/core/types/keyboard.types';
import { IDisplay } from '@/core/interfaces/display.interfaces';
import { ButtonHelper } from '../../core/helpers/button.helper';
...
class SimpleKeyboard {
  input!: IKeyboardInput;
  options!: IKeyboardOptions;
  utilities: any;
  caretPosition!: number;
  caretPositionEnd!: number;
  keyboardDOM!: TKeyboardElement;
  keyboardPluginClasses!: string;
  keyboardDOMClass!: string;
  buttonElements!: IKeyboardButtonElements;
  ...
  keyboardRowsDOM!: TKeyboardElement;
  defaultName = `default`;
   ...
   * @param {IKeyboardOptions} keyboardOptions? object defining the behaviour of the Keyboard
   */
  constructor(
    selectorOrOptions?: {
      debug: boolean;
      ...
      onKeyPress: (button: string, e: (Event | undefined)) => void;
    ...
    /**
     * Initializing InputUtilities
     */
    this.utilities = new InputUtilities({
     ...
     * @property {number} maxLength Restrains all of simple-keyboard inputs to a certain length. This should be used in addition to the input element’s maxlength attribute.
     * @property {object} maxLength Restrains simple-keyboard’s individual inputs to a certain length. This should be used in addition to the input element’s maxlength attribute.
     ...
     * Bindings
     */
    InputUtilities.bindMethods(SimpleKeyboard, this);
    ...
    window.SimpleKeyboardInstance = this;
   ...
   */
  handleParams = (
    selectorOrOptions?: string | HTMLDivElement | IKeyboardOptions,
    keyboardOptions?: IKeyboardOptions,
  ): {
    keyboardDOMClass: string;
    keyboardDOM: TKeyboardElement;
    options: Partial<IKeyboardOptions | undefined>;
  } => {
   ...
   * Getters
   */
  getOptions = (): IKeyboardOptions => this.options;
   ...
   * @param {number} endPosition The caret's end position
   */
   ...
   * @param candidateKey The chosen candidates string as defined in the layoutCandidates option
    ...
    if(this.candidateBox) {
      this.candidateBox.show({
        ...
        onSelect: (selectedCandidate: string, e: MouseEvent) => {
          ...
          const currentInput = this.getInput(this.options.inputName);
          const initialCaretPosition = this.getCaretPositionEnd() || 0;
          const inputSubstr = currentInput.substring(0, initialCaretPosition || 0)
            || currentInput;
          ...
          this.setInput(newInput, this.options.inputName);
          this.setCaretPosition(newCaretPosition);

          if(typeof this.options.onChange === `function`) {
            this.options.onChange(
              this.getInput(this.options.inputName),
              e,
            );
          }
        },
   ...
   * @param  {TKeyboardHandlerEvent} e
   */
  handleButtonClicked(button: string, e?: TKeyboardHandlerEvent): void {
     ...
     */
    if(ButtonHelper.isStandardButton(button) && this.activeInputElement) {
      const isEntireInputSelection = this.input[inputName]
        && this.input[inputName] === updatedInput
        && this.caretPosition === 0
        && this.caretPositionEnd === updatedInput.length;

      if(isEntireInputSelection) {
        this.setInput(``, this.options.inputName);
    ...
    if(
       ...
       * Updating input
       */
      const newInputValue = this.utilities.getUpdatedInput(button, this.input[inputName], this.caretPosition, this.caretPositionEnd, true);

      this.setInput(newInputValue, this.options.inputName);
       ...
       */
      if(typeof this.options.onChange === `function`) {
        this.options.onChange(this.getInput(this.options.inputName), e);
      }
    ...
    if(debug) {
      // eslint-disable-next-line no-console
      console.debug(`Key pressed:`, button);
    }
  }
  ...
  handleButtonMouseDown(button: string, e: TKeyboardHandlerEvent): void {
    if(e) {
      /**
    ...
    if(!this.options.disableButtonHold) {
      this.holdTimeout = window.setTimeout(() => {
        if(
          ...
          if(this.options.debug) {
            // eslint-disable-next-line no-console
            console.debug(`Button held:`, button);
          }
   ...
   */
  handleButtonMouseUp(button?: string, e?: TKeyboardHandlerEvent): void {
    if(e) {
      /**
       * Handle event options
       */
      ...
      /* istanbul ignore next */
      const isKeyboard = e.target === this.keyboardDOM
   ...
   */
  handleKeyboardContainerMouseDown(e: TKeyboardHandlerEvent): void {
    /**
     * Handle event options
     */
    if(this.options.preventMouseDownDefault) e.preventDefault();
   ...
   */
  replaceInput(inputObj: IKeyboardInput): void {
    this.input = inputObj;
  }
  ...
  setOptions(options = {}): void {
    ...
    if(changedOptions.length) {
      if(this.options.debug) {
        // eslint-disable-next-line no-console
        console.debug(`changedOptions`, changedOptions);
      }
   ...
   */
  changedOptions(newOptions: Partial<IKeyboardOptions>): string[] {
    return Object.keys(newOptions).filter(
      optionName => JSON.stringify(newOptions[optionName])
   ...
   * @param  {object} changedOptions The options to set
   */
  ...
  dispatch(callback: (instance: SimpleKeyboard, key?: string) => void): void {
    if(!window.SimpleKeyboardInstance) {
      console.warn(
        `SimpleKeyboardInstance is not defined. Dispatch cannot be called.`,
      );
      throw new Error(`INSTANCES_VAR_ERROR`);
    }
    callback(window.SimpleKeyboardInstance);
  }
  ...
  getButtonElement(
    button: string,
  ): TKeyboardElement | TKeyboardElement[] | undefined {
  ...
  inputPatternIsValid(inputVal: string): boolean {
    ...
    if(this.options.inputPatternRaw === undefined) {
      /**
       * inputPattern doesn't seem to be set for the current input. Pass.
       */
      return true;
    }
    let inputPattern;
   ...
   * Event Handler: KeyUp
   */
  handleKeyUp(event: TKeyboardHandlerEvent): void {
   ...
   */
  handleKeyDown(event: TKeyboardHandlerEvent): void {
    if(this.options.physicalKeyboardHighlight) {
   ...
   */
  handleMouseUp(event: TKeyboardHandlerEvent): void {
    this.caretEventHandler(event);
  }
   ...
   */
  /* istanbul ignore next */
  handleTouchEnd(event: TKeyboardHandlerEvent): void {
    this.caretEventHandler(event);
  }
   ...
   */
  handleSelect(event: TKeyboardHandlerEvent): void {
    this.caretEventHandler(event);
  }
   ...
   */
  /* istanbul ignore next */
  handleSelectionChange(event: TKeyboardHandlerEvent): void {
    this.caretEventHandler(event);
  }
   ...
   */
  caretEventHandler(event: TKeyboardHandlerEvent): void {
    ...
    this.dispatch(instance => {
      let isKeyboard = event.target === instance.keyboardDOM
  ...
  destroy(): void {
     ...
     * Remove buttons
     */
    const deleteButton = (buttonElement: TKeyboardElement | null): void => {
      if(buttonElement) {
    ...
    delete window.SimpleKeyboardInstance;
  ...
  setDOMButtonAttributes(button: string, callback: any): void {
    ...
    if(Array.isArray(buttonAttributes)) {
      buttonAttributes.forEach(attrObj => {
        if(
          attrObj.attribute && attrObj.value
          && attrObj.buttons
        ) {
   ...
   */
  /* istanbul ignore next */
  disableContextualWindow(): boolean {
    window.oncontextmenu = (event: TKeyboardHandlerEvent): boolean | undefined => {
  ...
  onInit(): void {
    if(this.options.debug) {
      // eslint-disable-next-line no-console
      console.debug(`${this.keyboardDOMClass} Initialized`);
    }
  ...
  beforeFirstRender(): void {
    ...
    if(
      ...
      if(this.options.debug) {
        // eslint-disable-next-line no-console
        console.debug(`Using PointerEvents as it is supported by this browser`);
      }
    }
  ...
  beforeRender(): void {
    if(typeof this.options.beforeRender === `function`) {
      ...
      if(this.options.debug) {
        // eslint-disable-next-line no-console
        console.debug(`beforeRender was triggered.`);
      }
    }
  }
  ...
  onRender(): void {
    if(typeof this.options.onRender === `function`) {
      ...
      if(this.options.debug) {
        // eslint-disable-next-line no-console
        console.debug(`onRender was triggered.`);
      }
    }
  }
  ...
  onModulesLoaded(): void {
    if(typeof this.options.onModulesLoaded === `function`) {
      ...
      if(this.options.debug) {
        // eslint-disable-next-line no-console
        console.debug(`onModulesLoaded was triggered.`);
      }
    }
  }
  ...
  registerModule = (name: string, initCallback: any): void => {
    if(!this.modules[name]) {
      ...
      if(this.options.debug) {
        // eslint-disable-next-line no-console
        console.debug(`registerModule was triggered.`);
      }
    }
  ...
  loadModules(): void {
    if(Array.isArray(this.options.modules)) {
      ...
      if(this.options.debug) {
        // eslint-disable-next-line no-console
        console.debug(`modules-loaded completed.`);
      }
      this.render();
   ...
   * Get module prop
   */
  getModuleProp(name: string, prop: string): boolean {
  ...
  parseRowDOMContainers(
    ...
    if(rowDOMArray.length) {
      containerStartIndexes.forEach((startIndex, arrIndex): boolean | undefined => {
  ...
  render(): void {
    ...
    const layout = this.options.layout || getDefaultLayout().layout;
      ...
      (row, rIndex) => {
        ...
        if(
          ...
          rowArray = rowArray.filter(
            buttonName => this.options.excludeFromLayout
              && !this.options.excludeFromLayout[
                this.options.layoutName || this.defaultName
              ].includes(buttonName),
          );
         ...
         * Creating empty row
         */
        let rowDOM: HTMLDivElement = document.createElement(`div`);
        ...
        rowArray.forEach((button, bIndex) => {
           ...
           */
          const buttonHasContainerStart = !disableRowButtonContainers
            && button.length > 1
            && button.indexOf(`[`) === 0;

          const buttonHasContainerEnd = !disableRowButtonContainers
            && button.length > 1
           ...
           */
          const fctBtnClass = ButtonHelper.getButtonClass(button);
          const buttonDisplayName = ButtonHelper.getButtonDisplayName(
            button,
          ...
          this.setDOMButtonAttributes(
            button, (attribute: string, value: string) => {
          ...
          if(
             ...
             */
            buttonDOM.onpointerdown = (e: TKeyboardHandlerEvent): void => {
              this.handleButtonClicked(button, e);
              this.handleButtonMouseDown(button, e);
            };
            buttonDOM.onpointerup = (e: TKeyboardHandlerEvent): void => {
              this.handleButtonMouseUp(button, e);
            };
            buttonDOM.onpointercancel = (e: TKeyboardHandlerEvent): void => {
              this.handleButtonMouseUp(button, e);
            };
          } else {
            /**
            ...
            if(useTouchEvents) {
               ...
               * Handle touch events
               */
              buttonDOM.ontouchstart = (e: TKeyboardHandlerEvent): void => {
                this.handleButtonClicked(button, e);
                this.handleButtonMouseDown(button, e);
              };
              buttonDOM.ontouchend = (e: TKeyboardHandlerEvent): void => {
                this.handleButtonMouseUp(button, e);
              };
              buttonDOM.ontouchcancel = (e: TKeyboardHandlerEvent): void => {
                this.handleButtonMouseUp(button, e);
              };
            } else {
              /**
               * Handle mouse events
               */
              buttonDOM.onclick = (e: TKeyboardHandlerEvent): void => {
                this.setMouseHold(false);
                /**
              ...
              buttonDOM.onmousedown = (e: TKeyboardHandlerEvent): void => {
                /**
              ...
              buttonDOM.onmouseup = (e: TKeyboardHandlerEvent): void => {
                this.handleButtonMouseUp(button, e);
              };
            }
          }
    ...
    if(!this.initialized) {
      ...
      if(
        ...
        document.onpointerup = (e: TKeyboardHandlerEvent): void => {
          this.handleButtonMouseUp(undefined, e);
        };
        this.keyboardDOM.onpointerdown = (e: TKeyboardHandlerEvent): void => {
         ...
         */
        document.ontouchend = (e: TKeyboardHandlerEvent): void => {
          this.handleButtonMouseUp(undefined, e);
        };
        document.ontouchcancel = (e: TKeyboardHandlerEvent): void => {
          this.handleButtonMouseUp(undefined, e);
        };

        this.keyboardDOM.ontouchstart = (e: TKeyboardHandlerEvent): void => {
          this.handleKeyboardContainerMouseDown(e);
        };
      } else if(!useTouchEvents) {
        /**
         * Handling mouseup
         */
        document.onmouseup = (e: TKeyboardHandlerEvent): void => {
          this.handleButtonMouseUp(undefined, e);
        };
        this.keyboardDOM.onmousedown = (e: TKeyboardHandlerEvent): void => {
          this.handleKeyboardContainerMouseDown(e);
        };
      }
